...
const ChessGameWrapper = (props) => {
    ...
    React.useEffect(() => {
        ...
        // This is the key event for starting the game
        socket.on('start game', (opponentUserName) => {
            console.log("START!")
            // Force set didJoinGame to true regardless of which player we are
            didJoinGame(true)
            
            if (opponentUserName !== props.myUserName) {
                setUserName(opponentUserName)
            } else {
                // Request username from opponent if we're the original creator
        ...
        socket.on('get Opponent UserName', (data) => {
            if (socket.id !== data.socketId) {
                ...
                // Ensure we set didJoinGame to true here as well
                didJoinGame(true)
            }
        })
